  ...
  persist(
    (set, get) => ({
      ...
      updateGuestExclusions: (guestId, excludedGuestIds) => {
        set((state) => {
          // Remove all existing exclusions for this guest
      ...
      updateConfiguration: (updates) => {
        set((state) => {
          // tableShape is only visual, doesn't require regeneration
      ...
      // Remove excess auto-generated guests
      removeExcessAutoGeneratedGuests: (count) => {
        const { guests, couples, exclusions } = get();

        // Identify auto-generated guests (those starting with "Invité")
      ...
      generate: () => {
        ...
        // Save custom table names before regeneration
        const customTableNames = new Map<number, string>();
        if (previousResult) {
          for (const table of previousResult.tables) {
            // Check if name has been customized (different from default)
        ...
        // Initialize table positions for visual view
        // Honor table centered at top, other tables in grid below
        const tablePositions: TablePosition[] = result.tables.map((table, index) => {
          if (index === 0) {
            // Honor table centered at top
            return {
              tableId: table.id,
      ...
      moveGuestToTable: (guestId, targetTableId) => {
        set((state) => {
          ...
          // Check if guest is already in target table
